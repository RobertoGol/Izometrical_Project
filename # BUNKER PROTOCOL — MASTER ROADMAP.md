...
## P8: Архитектура 3D-Движка и Базовый Рендеринг (Core 3D Engine)
*Глобальный переход на полноценное 3D в духе Fallout 4/76 (Ветка `Update-m157-base-edi`)*

- [ ] **Инициализация графического конвейера (SDL3):** Настройка контекста под 3D-рендер, Z-буферизация (Depth Test), настройка матриц перспективной проекции.
- [ ] **Камера (1st/3rd Person):** Матричная математика (View/Projection matrices), расчет векторов направления (Front, Up, Right) для свободного обзора и перемещения по миру.
- [ ] **Загрузчик ассетов (3D Меши):** Парсер 3D-моделей (.obj/.gltf) и архитектура чтения запакованных игровых архивов (по аналогии с форматом .ba2).
- [ ] **Освещение и материалы:** Базовые шейдеры для направленного (Directional) и точечного (Point) освещения, подготовка материалов для моделей.
- [ ] **Интерактивные 3D-сущности:** Интеграция хитбоксов (AABB/OBB) для контейнеров, сейфов и предметов, с которыми можно взаимодействовать в мире.

## P9: Режим Мастерской и Диегетический Интерфейс (Workshop & FUI)
*Механики строительства баз, логистика и интеграция терминала PIP-PAD 3000*

- [ ] **Система Raycasting:** Бросок луча из центра камеры для точного определения координат поверхности и пересечений с объектами (основа для режима строительства).
- [ ] **3D-Снаппинг (Socket System):** Логика математической привязки модулей базы (полы к полам, стены к краям, коннекторы питания) к строительной сетке.
- [ ] **Валидация строительства:** Проверка коллизий (зеленый/красный голографический материал предпросмотра при установке) и сериализация данных (сохранение/загрузка базы).
- [ ] **Диегетический UI (PIP-PAD 3000):** Рендеринг интерфейса (ImGui) в текстуру (Render-to-Texture) и ее динамическое проецирование на экран 3D-модели терминала на руке.
- [ ] **Quick-Loot FUI:** Всплывающий в 3D-пространстве виджет быстрого обыска контейнеров (в реальном времени, без паузы игры).
