...
inline float TextureGenerator::perlinNoise(float x, float y, int seed)
        {
...
inline float TextureGenerator::fbm(float x, float y, int octaves, int seed)
        {
            float value = 0.0f;
...
inline std::uint8_t TextureGenerator::clampByte(int v)
        {
            return static_cast<std::uint8_t>(std::max(0, std::min(255, v)));
        }

inline bool TextureGenerator::needsGeneration(const std::string& outputDir, int expectedCount) const
        {
...
inline bool TextureGenerator::generate(const TextureGenConfig& cfg)
        {
...
inline void TextureGenerator::generateAllOnFirstRun()
        {
            ...
            {
                generate({"assets/textures/base_enemy.png",
...
inline bool TextureGenerator::loadBase(const std::string& path, sf::Image& out)
        {
...
inline void TextureGenerator::applyRecolor(sf::Image& img, const sf::Image& mask, bool hasMask, int seed)
        {
...
inline void TextureGenerator::applyTerrainNoise(sf::Image& img, int seed)
        {
            unsigned w = img.getSize().x;
            unsigned h = img.getSize().y;
...
inline void TextureGenerator::applyCreatureVariation(sf::Image& img, const sf::Image& mask, bool hasMask, int seed)
        {
...
inline void TextureGenerator::applyRobotScheme(sf::Image& img, const sf::Image& mask, bool hasMask, int seed)
        {
...
inline void TextureGenerator::applyItemVariation(sf::Image& img, const sf::Image& mask, bool hasMask, int seed)
        {
            applyRecolor(img, mask, hasMask, seed);
...
inline sf::Image TextureGenerator::generateNormalMap(const sf::Image& source, float strength)
        {
