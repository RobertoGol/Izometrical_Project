...
public:
    ...
    static DamageResult applyPlayerDamage(GameState& gs, float rawDamage, DamageType type) {
        DamageResult res;
        if (!gs.isRunning || gs.playerHealth <= 0.0f) return res;

        // Модификаторы сопротивлений Пилота
        float multiplier = 1.0f;
        switch (type) {
            case DamageType::Environmental: multiplier = 0.75f; break;
            case DamageType::Acid:          multiplier = 1.20f; break;
            case DamageType::Explosive:     multiplier = 1.10f; break;
            default: break;
        }
    ...
    static DamageResult applyTitanDamage(GameState& gs, float rawDamage, DamageType type) {
        DamageResult res;
        if (gs.titan.health <= 0.0f) return res;

        float dmg = rawDamage;
        if (type == DamageType::Thermal) dmg *= 0.70f; // Тяжёлая термозащита реактора
        if (type == DamageType::Acid)    dmg *= 1.30f; // Разъедание гидравлики
    ...
    static void applyTowerDamage(GameState& gs, float rawDamage) {
        if (gs.regionalGrid.towerHealth <= 0.0f) return;
    ...
    static void applyRadiusDamage(GameState& gs, const Vector3D& origin, float maxRadius, 
        ...
        if (maxRadius <= 0.001f || maxDamage <= 0.0f) return;
