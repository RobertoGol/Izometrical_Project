...
//
// Этот файл специально сделан header-only, чтобы его можно было просто положить
// в include/ и подключить из main.cpp без изменения CMakeLists.txt.
//
// Здесь собраны НЕ заглушки, а рабочие модели состояния/логики, которые дальше
// подключаются в главный цикл:
//   1) Weather: EtherFog / AcidRain / Clear
//   2) Radio/Tape: радиоперехваты и найденные .tape записи
//   3) Rations/Heal/Reload: расходники, баффы, лечение, перезарядка
//   4) Tank utilities: Bucket Rig / Ram Shield / Tow Coupler, seat swap,
//      hangar repair, thermal load
//   5) Stress / SecondWind / SoulLine
//   6) Reactive breakables + shock waves
//   7) Story route / zone events
//   8) Skill system
//   9) Loot generator by tiers
//  10) C.A.M.P. строительство
//  11) SpatialGrid для оптимизации поиска рядом
//  12) ToolGun + Prefab library + Undo/Redo
//  13) OBJ loader без внешних библиотек
//  14) LanlineServices: локальная модель lobby/chat/voice/delivery
//  15) Profile/session migration
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    void update(float dt) {
        ...
        for (auto& msg : m_Radio) {
            if (!msg.fired && m_Time >= msg.atTime) {
                ...
                if (m_Log.size() > 12) m_Log.pop_front();
                break;
            }
        }
    }
...
enum class RationKind { Protein, Stamina, Focus, AntiErosion };
...
public:
    ...
    bool useStim(GameState& gs, PlayerInventory& inv) {
        if (!inv.hasItem(ITEM_STIM) || gs.playerHealth >= gs.playerMaxHealth) return false;
    ...
    bool eatRation(GameState& gs, PlayerInventory& inv, RationKind kind) {
        ...
        switch (kind) {
            case RationKind::Protein: id = ITEM_RATION_PROTEIN; amount = 12.0f; duration = 60.0f; break;
            case RationKind::Stamina: id = ITEM_RATION_STAMINA; amount = 0.35f; duration = 50.0f; break;
            case RationKind::Focus: id = ITEM_RATION_FOCUS; amount = 0.20f; duration = 35.0f; break;
            case RationKind::AntiErosion: id = ITEM_RATION_ANTI_EROSION; amount = 25.0f; duration = 40.0f; break;
        }
        if (!inv.hasItem(id)) return false;
        inv.removeItem(id, 1);
    ...
    void startReload(PlayerInventory& inv) {
        if (m_Weapon.isReloading || m_Weapon.magazine >= m_Weapon.magazineMax) return;
        if (m_Weapon.reserveAmmo <= 0 && !inv.hasItem(ITEM_AMMO_556)) return;
        if (m_Weapon.reserveAmmo <= 0 && inv.removeItem(ITEM_AMMO_556, 1)) m_Weapon.reserveAmmo += 30;
        m_Weapon.isReloading = true;
        m_Weapon.reloadTimer = reloadDuration();
    }

    bool consumeRound() {
        if (m_Weapon.isReloading || m_Weapon.magazine <= 0) return false;
        --m_Weapon.magazine;
        return true;
    }
    ...
    float speedMultiplier() const {
        float result = 1.0f;
        for (const auto& b : m_Buffs) if (b.kind == RationKind::Stamina) result += b.amount;
        if (m_Stress > 70.0f) result -= 0.15f;
        return advClamp(result, 0.55f, 1.75f);
    }

    float aimPenalty() const {
        float penalty = (m_Stress > 35.0f) ? (m_Stress - 35.0f) / 650.0f : 0.0f;
        for (const auto& b : m_Buffs) if (b.kind == RationKind::Focus) penalty -= b.amount;
...
private:
    ...
    float reloadDuration() const {
        float d = 1.45f;
        for (const auto& b : m_Buffs) if (b.kind == RationKind::Focus) d *= 0.82f;
        return d;
    }
    ...
    void updateStress(GameState& gs, float dt) {
        ...
        if (nearEnemy || gs.playerHealth < gs.playerMaxHealth * 0.35f) addStress((nearEnemy ? 4.0f : 2.0f) * dt);
        else calm(2.5f * dt);
    }
    ...
    void updateBuffs(GameState& gs, float dt) {
        ...
        m_Buffs.erase(std::remove_if(m_Buffs.begin(), m_Buffs.end(), [](const ActiveBuff& b){ return b.timeLeft <= 0.0f; }), m_Buffs.end());
    }
...
enum class TankUtilityMode { BucketRig, RamShield, TowCoupler };
enum class TankSeat { Driver, Gunner };
...
public:
    ...
    void update(GameState& gs, float dt) {
        ...
        if (m_Runtime.cannonThermalLoad < 62.0f) m_Runtime.overheated = false;
        ...
        if (m_Runtime.towing) {
            ...
            if (toTank.lengthSq() < 0.35f) m_Runtime.towing = false;
        }
    }

    void nextUtility() {
        if (m_Runtime.utility == TankUtilityMode::BucketRig) m_Runtime.utility = TankUtilityMode::RamShield;
        else if (m_Runtime.utility == TankUtilityMode::RamShield) m_Runtime.utility = TankUtilityMode::TowCoupler;
        else m_Runtime.utility = TankUtilityMode::BucketRig;
    }
    ...
    bool useUtility(GameState& gs) {
        if (m_Runtime.utilityCooldown > 0.0f) return false;
        switch (m_Runtime.utility) {
            case TankUtilityMode::BucketRig:
                // Ковш разгребает стены/мусор перед танком.
                carveFront(gs, 2, 35);
                m_Runtime.utilityCooldown = 1.0f;
                return true;
            case TankUtilityMode::RamShield:
                // Таранный щит временно чинит гусеницы/корпус и даёт контактный урон.
    ...
    bool registerCannonShot(GameState& gs, float heat) {
        if (m_Runtime.overheated) return false;
    ...
    bool repairInHangar(GameState& gs, PlayerInventory& inv) {
        if (!isInsideHangar(gs.titan.position)) return false;
        if (!inv.removeItem(ITEM_REPAIR_KIT, 1)) return false;
    ...
    void carveFront(GameState& gs, int radius, int damage) {
        ...
        for (int x = cx - radius; x <= cx + radius; ++x) {
            for (int y = cy - radius; y <= cy + radius; ++y) {
                if (x < 0 || y < 0 || x >= Config::MAP_WIDTH || y >= Config::MAP_HEIGHT) continue;
                if (gs.sectorMap[x][y] == 1) {
                    gs.wallDurability[x][y] -= damage;
                    if (gs.wallDurability[x][y] <= 0) gs.sectorMap[x][y] = 0;
                }
            }
        }
    }
...
enum class BreakableKind { Glass, Vegetation, Crate, Barrel, Console };
...
public:
    int add(BreakableKind kind, Vector3D pos) {
        ...
        b.position = pos;
        switch (kind) {
            case BreakableKind::Glass: b.health = 8.0f; b.radius = 0.30f; break;
            case BreakableKind::Vegetation: b.health = 12.0f; b.radius = 0.45f; break;
            case BreakableKind::Crate: b.health = 35.0f; b.radius = 0.42f; break;
            case BreakableKind::Barrel: b.health = 25.0f; b.radius = 0.40f; break;
            case BreakableKind::Console: b.health = 55.0f; b.radius = 0.55f; break;
        }
    ...
    void update(GameState& gs, float dt) {
        ...
        m_Waves.erase(std::remove_if(m_Waves.begin(), m_Waves.end(), [](const ShockWave& w){ return w.ttl <= 0.0f; }), m_Waves.end());

        for (auto& b : m_Breakables) {
            if (b.broken) continue;
            b.position += b.velocity * dt;
            b.velocity = b.velocity * std::pow(0.04f, dt);
            if (b.kind == BreakableKind::Barrel && b.health <= 0.0f) explodeBarrel(gs, b);
            else if (b.health <= 0.0f) b.broken = true;
        }
    }
...
private:
    ...
    void applyWave(GameState& gs, const ShockWave& wave, float dt) {
        ...
        for (auto& e : gs.enemies) {
            ...
            if (de >= inner && de <= outer) e.position += advNormalize2D(e.position - wave.origin) * (wave.force * 0.8f * dt);
        }
    }
};
...
enum class ZoneEventId { CryoLocker, Archive, Garage, Exterior, FirstCombat, ReturnToBase, SurfaceClearance };
...
private:
    ...
    void rebuildRoute() {
        m_Route.clear();
        for (const auto& t : m_Triggers) m_Route.push_back({t.objective, t.fired});
    }

    void mark(const std::string& text) {
        for (auto& r : m_Route) if (r.text == text) r.completed = true;
    }
    ...
    void fire(GameState& gs, ZoneTrigger& t, RadioTapeSystem* radio) {
        ...
        switch (t.id) {
            case ZoneEventId::CryoLocker:
                ...
                if (radio) radio->discoverTape("CRYO_00");
                break;
            ...
            case ZoneEventId::Garage:
                ...
                if (radio) radio->discoverTape("GARAGE_BT");
                break;
            ...
            case ZoneEventId::Exterior:
                ...
                if (radio) radio->discoverTape("SURFACE_17");
                break;
...
enum class SkillId { ArchiveSync, FootKill, TankAction, StressSurvival, Mechanic, Builder };
...
public:
    ...
    bool upgrade(GameState& gs, SkillId id) {
        if (gs.characterProg.unusedPoints <= 0) return false;
    ...
    int rank(SkillId id) const { auto it = m_Skills.rank.find(id); return it == m_Skills.rank.end() ? 0 : it->second; }
    ...
    void applyPassive(GameState& gs, SkillId id, int r) {
        switch (id) {
            case SkillId::ArchiveSync:
                ...
                if (r >= 3) gs.regionalGrid.towerSyncRecovered = true;
                break;
...
enum class LootTier { Common, Uncommon, Rare, Epic, Legendary };
...
class LootGenerator {
public:
    LootGenerator() : m_Rng(0x17A55E7u) {
        addDefaults();
    }

    InventoryItem roll(LootTier tier) {
        auto& table = m_Tables[tier];
        if (table.empty()) return {1, ItemType::Resource, 1, 0.1f, "SCRAP"};
        float total = 0.0f;
    ...
    void fillContainer(LootContainer& c, LootTier tier, int rolls) {
        ...
        for (int i = 0; i < rolls; ++i) c.containsItems.push_back(roll(tier));
    }

    void normalizeWorldLoot(GameState& gs) {
        int i = 0;
        for (auto& c : gs.lootContainers) {
            if (!c.containsItems.empty()) continue;
            LootTier tier = LootTier::Common;
            if (c.type == LootContainerType::IronSafe) tier = LootTier::Rare;
            if (c.type == LootContainerType::DevVault) tier = LootTier::Legendary;
...
public:
    ...
    void cycleType() {
        if (m_Preview.activeType == CampObjectType::ConcreteWall) m_Preview.activeType = CampObjectType::DefenseTurret;
        else if (m_Preview.activeType == CampObjectType::DefenseTurret) m_Preview.activeType = CampObjectType::SupplyCrate;
        else m_Preview.activeType = CampObjectType::ConcreteWall;
    }
    ...
    bool place(GameState& gs, PlayerInventory& inv, float costMult = 1.0f) {
        if (!m_Enabled || !m_Preview.isPlacementValid) return false;
        const int cost = std::max(1, static_cast<int>(baseCost(m_Preview.activeType) * costMult));
        if (!inv.removeItem(ITEM_BUILD_MATERIAL, cost)) return false;
    ...
    void updateTurrets(GameState& gs, float dt) {
        ...
        for (const auto& obj : m_Objects) {
            if (obj.type != CampObjectType::DefenseTurret) continue;
...
private:
    ...
    static int baseCost(CampObjectType type) {
        switch (type) {
            case CampObjectType::ConcreteWall: return 4;
            case CampObjectType::DefenseTurret: return 12;
            case CampObjectType::SupplyCrate: return 6;
        }
        return 5;
    }

    bool canPlace(const GameState& gs, int x, int y) const {
        if (x < 0 || y < 0 || x >= Config::MAP_WIDTH || y >= Config::MAP_HEIGHT) return false;
        if (gs.sectorMap[x][y] == 1) return false;
        if (advDistSq({x + 0.5f, y + 0.5f, 0.0f}, gs.playerPos) > 5.0f * 5.0f) return false;
        for (const auto& obj : m_Objects) if (obj.tileX == x && obj.tileY == y) return false;
        return true;
    }
};
...
class SpatialGrid {
public:
    explicit SpatialGrid(float cellSize = 2.0f) : m_CellSize(std::max(0.25f, cellSize)) {}
    ...
    void query(Vector3D pos, float radius, Fn fn) const {
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = minY; y <= maxY; ++y) {
                ...
                for (const auto& value : it->second) fn(value);
            }
        }
    }
...
enum class ToolGunMode { SpawnPrefab, Delete, PaintErosion, Validate, Export };
...
public:
    ...
    const PrefabDef* get(const std::string& name) const {
        for (const auto& p : m_Prefabs) if (p.name == name) return &p;
...
public:
    void cycleMode() {
        if (m_Mode == ToolGunMode::SpawnPrefab) m_Mode = ToolGunMode::Delete;
        else if (m_Mode == ToolGunMode::Delete) m_Mode = ToolGunMode::PaintErosion;
        else if (m_Mode == ToolGunMode::PaintErosion) m_Mode = ToolGunMode::Validate;
        else if (m_Mode == ToolGunMode::Validate) m_Mode = ToolGunMode::Export;
        else m_Mode = ToolGunMode::SpawnPrefab;
    }
    ...
    bool apply(GameState& gs, Vector3D where, const PrefabLibrary& lib) {
        ...
        if (x < 0 || y < 0 || x >= Config::MAP_WIDTH || y >= Config::MAP_HEIGHT) return false;
        switch (m_Mode) {
            case ToolGunMode::SpawnPrefab: return spawnPrefab(gs, x, y, lib);
            case ToolGunMode::Delete: return deleteTile(gs, x, y);
            case ToolGunMode::PaintErosion: return paintErosion(gs, x, y);
            case ToolGunMode::Validate: m_LastValidation = validate(gs); return true;
            case ToolGunMode::Export: m_LastExport = exportMap(gs); return true;
        }
        return false;
    }
...
private:
    ...
    bool spawnPrefab(GameState& gs, int x, int y, const PrefabLibrary& lib) {
        ...
        for (int row = 0; row < static_cast<int>(prefab->rows.size()); ++row) {
            for (int col = 0; col < static_cast<int>(prefab->rows[row].size()); ++col) {
                ...
                if (c == '#') { gs.sectorMap[tx][ty] = 1; gs.wallDurability[tx][ty] = 110; }
                else if (c == '.') { gs.sectorMap[tx][ty] = 0; gs.wallDurability[tx][ty] = 0; }
                else if (c == 'c') {
                    LootContainer lc;
        ...
        pushAction({"spawn prefab", [oldMap, oldDur](GameState& s){ s.sectorMap = oldMap; s.wallDurability = oldDur; },
                                   [newMap, newDur](GameState& s){ s.sectorMap = newMap; s.wallDurability = newDur; }});
        return true;
    }
    ...
    bool deleteTile(GameState& gs, int x, int y) {
        ...
        pushAction({"delete tile", [x, y, oldTile, oldDur](GameState& s){ s.sectorMap[x][y] = oldTile; s.wallDurability[x][y] = oldDur; },
                                  [x, y](GameState& s){ s.sectorMap[x][y] = 0; s.wallDurability[x][y] = 0; }});
        return true;
    }
    ...
    std::string validate(const GameState& gs) const {
        ...
        for (int x = 0; x < Config::MAP_WIDTH; ++x) {
            for (int y = 0; y < Config::MAP_HEIGHT; ++y) {
                if (gs.sectorMap[x][y] == 1) {
                    ++walls;
                    if (gs.wallDurability[x][y] <= 0) ++brokenWalls;
                }
            }
        }
        for (const auto& e : gs.enemies) {
            ...
            if (ex < 0 || ey < 0 || ex >= Config::MAP_WIDTH || ey >= Config::MAP_HEIGHT || gs.sectorMap[ex][ey] == 1) ++invalidEnemies;
        }
    ...
    std::string exportMap(const GameState& gs) const {
        ...
        for (int y = 0; y < Config::MAP_HEIGHT; ++y) {
            for (int x = 0; x < Config::MAP_WIDTH; ++x) out << (gs.sectorMap[x][y] == 1 ? '#' : '.');
            out << '\n';
        }
        return out.str();
    }
...
struct ObjFace { std::array<int, 3> v{{0, 0, 0}}; };
    ...
    static ObjModel load(const std::string& path) {
        ...
        while (std::getline(in, line)) {
            ...
            if (tag == "v") {
                ...
                if (idx[0] >= 0 && idx[1] >= 0 && idx[2] >= 0) model.faces.push_back({idx});
            }
        }
        return model;
    }
...
public:
    ...
    void sendChat(int fromPeer, const std::string& text) {
        ...
        if (m_Chat.size() > 20) m_Chat.erase(m_Chat.begin());
    }

    void setVoice(int peerId, bool active) {
        for (auto& p : m_Peers) if (p.id == peerId) p.voiceActive = active;
    }
    ...
    void update(GameState& gs, PlayerInventory& inv, float dt) {
        for (auto& c : m_Chat) c.ttl -= dt;
        m_Chat.erase(std::remove_if(m_Chat.begin(), m_Chat.end(), [](const LanlineChatMessage& c){ return c.ttl <= 0.0f; }), m_Chat.end());
        ...
        for (auto& d : m_Deliveries) {
            ...
            if (d.eta <= 0.0f) {
                d.delivered = true;
                if (d.payload == "ammo") inv.addItem(SurvivalSystem::ITEM_AMMO_556, ItemType::Ammo, 2, 0.15f, "5.56 AMMO BOX");
                else if (d.payload == "med") inv.addItem(SurvivalSystem::ITEM_STIM, ItemType::Medicine, 1, 0.10f, "STIM INJECTOR");
                else inv.addItem(201, ItemType::Resource, 10, 0.08f, "SCRAP METAL");
                gs.score += 25;
...
public:
    ...
    void update(GameState& gs, PlayerInventory& inv, const InputSnapshot& input, float dt) {
        ...
        // Интеграция доп. hotkeys через InputSnapshot + прямые поля уже будет
        // добавлена в main.cpp следующим файлом. Здесь оставляем безопасные
        // действия, не конфликтующие с текущим вводом.
        if (input.toggleCamp) camp.toggle();
        if (input.switchWeapon && gs.playerMode == UnitMode::Titan) tankUtility.nextUtility();
        if (input.dismountVehicle && gs.playerMode == UnitMode::Titan) tankUtility.swapSeat();
        if (input.interact) {
