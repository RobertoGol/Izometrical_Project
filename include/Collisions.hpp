...
enum class PhysicalMassClass {
    StaticWall,     // Бесконечная масса (стены бункера)
    VanguardTitan,  // 40 тонн (БТ-7274)
    HumanPilot,     // 100 кг (Пилот)
    VerminSwarm     // 15 кг (особь Роя)
};
...
class Collisions {
public:
    // ── Проверка столкновения игрока со стенами (AABB по 4 точкам хитбокса) ──
    ...
    // ── Разрешение столкновения двух кругов с учётом масс ──
    ...
    // ── Выталкивание круга из тайла стены ──
    ...
    // ── Полный обсчёт коллизий всех сущностей за кадр ──
        ...
        for (size_t i = 0; i < gs.enemies.size(); ++i) {
            ...
            if (resolveCircleVsCircle(playerBody, enemyBody)) {
                if (gs.playerMode == UnitMode::Scout) {
                    gs.playerHealth -= 2.0f * gs.deltaTime;  // Урон по dt, не за кадр!
                }
            }
