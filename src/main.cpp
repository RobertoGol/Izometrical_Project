...
void renderAdvancedWorld(sf::RenderWindow &window, const bunker::AdvancedMechanics &adv)
{
    ...
    for (const auto &b : adv.reactive.breakables())
    {
        ...
        switch (b.kind)
        {
        case bunker::BreakableKind::Glass:
            color = sf::Color(125, 210, 255, 150);
            points = 4;
            radius = 7.0f;
            break;
        case bunker::BreakableKind::Vegetation:
            color = sf::Color(75, 170, 70);
            points = 7;
            radius = 9.0f;
            break;
        case bunker::BreakableKind::Crate:
            color = sf::Color(170, 120, 55);
            points = 4;
            radius = 9.0f;
            break;
        case bunker::BreakableKind::Barrel:
            color = sf::Color(210, 75, 45);
            points = 12;
            radius = 8.0f;
            break;
        case bunker::BreakableKind::Console:
...
void renderAdvancedHUD(sf::RenderWindow &window, const bunker::AdvancedMechanics &adv, const sf::Font *font)
{
    ...
    if (tank.utility == bunker::TankUtilityMode::RamShield)
        util = "RamShield";
    if (tank.utility == bunker::TankUtilityMode::TowCoupler)
        util = "TowCoupler";
...
// ═══════════════════════════════════════════════════════
// MAIN (Legacy copy preserved for reference)
// ═══════════════════════════════════════════════════════
int legacy_main_bunker_iso()
{
    ...
    for (std::size_t i = 0; i < gs.enemies.size(); ++i)
    {
        bunker::HostileKind kind = bunker::HostileKind::VerminRush;
        if (i % 4 == 1)
            kind = bunker::HostileKind::GhoulRush;
        if (i % 4 == 2)
            kind = bunker::HostileKind::HumanTactical;
        if (i % 4 == 3)
            kind = bunker::HostileKind::RobotControl;
    ...
    while (window.isOpen() && gs.isRunning)
    {
        ...
        if (mapKeyNow && !prevMapKey)
            mapScreen.toggle();
        if (timeShiftKeyNow && !prevTimeShiftKey)
            timeShift.tryShift(gs);

        // Advanced hotkeys. Всё edge-triggered, чтобы не срабатывало 60 раз/сек.
        if (healKeyNow && !prevHealKey)
            adv.survival.useStim(gs, inventory);
        if (rationKeyNow && !prevRationKey)
            adv.survival.eatRation(gs, inventory, bunker::RationKind::Stamina);
        if (reloadKeyNow && !prevReloadKey && gs.playerMode == bunker::UnitMode::Scout)
            adv.survival.startReload(inventory);
        if (campCycleKeyNow && !prevCampCycleKey)
            adv.camp.cycleType();
        if (useUtilityKeyNow && !prevUseUtilityKey && gs.playerMode == bunker::UnitMode::Titan)
            adv.tankUtility.useUtility(gs);
        if (tapeKeyNow && !prevTapeKey)
            std::cout << adv.radio.playNextUnplayed() << std::endl;
        if (toolModeKeyNow && !prevToolModeKey)
            adv.toolgun.cycleMode();
        if (toolUseKeyNow && !prevToolUseKey && inventory.hasItem(777))
            adv.toolgun.apply(gs, gs.mouseWorldPos, adv.prefabs);
        if (undoKeyNow && !prevUndoKey)
            adv.toolgun.undo(gs);
        if (redoKeyNow && !prevRedoKey)
            adv.toolgun.redo(gs);
        if (deliveryKeyNow && !prevDeliveryKey)
            adv.lanline.requestDelivery("ammo", gs.playerPos);
        ...
        if (gs.playerMode == bunker::UnitMode::Scout && !vehicleMgr.isPlayerInVehicle())
        {
            ...
            for (int i = 0; i < 7; ++i)
            {
                if (input.selectClass[i])
                    tactics.changePilotClass(classes[i]);
            }
        }
        ...
        if (input.activateTactical && gs.playerMode == bunker::UnitMode::Scout && !vehicleMgr.isPlayerInVehicle())
        {
