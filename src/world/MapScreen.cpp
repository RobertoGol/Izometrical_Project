...
void MapScreen::collectMarkers(const GameState& gs) {
    ...
    m_Markers.push_back({
        gs.playerPos, "You", sf::Color(50, 255, 50),
        MapMarker::Shape::Triangle, 7.0f, false
    });

    m_Markers.push_back({
        gs.titan.position, "BT-7274", sf::Color(230, 115, 25),
        MapMarker::Shape::Diamond, 8.0f, gs.titan.health < 300.0f
    });

    m_Markers.push_back({
        gs.towerPosition, "Relay Tower", sf::Color::White,
    ...
    for (const auto& e : gs.enemies) {
        ...
        m_Markers.push_back({
            e.position, "", sf::Color(255, 50, 50, 200),
    ...
    for (const auto& c : gs.lootContainers) {
        ...
        m_Markers.push_back({
            c.position, "", sf::Color(180, 140, 60),
