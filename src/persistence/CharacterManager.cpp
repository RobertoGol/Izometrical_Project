...
namespace bunker
{
    ...
    void CharacterManager::restoreSanityAndSoul(GameState& gs, float amount)
    {
        ...
        bunker::logInfo() << "[SANITY & SOUL] Линии Разума и Души стабилизированы: " << gs.characterProg.sanityLine
                          << "% / " << gs.characterProg.soulLine << "%" << std::endl;
    }
    ...
    void CharacterManager::checkConsciousnessTrappingFailSafe(GameState& gs)
    {
        ...
        if (gs.playerErosionLevel >= 95.0f)
        {
            ...
            if (gs.characterProg.sanityLine <= 0.0f && gs.characterProg.soulLine > 0.0f)
            {
                ...
                bunker::logInfo()
                    << "[META FAIL-SAFE] !! ПРЕДОХРАНИТЕЛЬ СОЗНАНИЯ !! Линия Души компенсирует распад воспоминаний!"
                    << std::endl;
            }
        }
    }
