...
bool TitanAI::tryMountGunner(GameState& gs) {
    if (m_IsGunnerMounted) return false;
...
void TitanAI::cycleCockpitFireMode() {
    if (m_IsGunnerMounted) return; // В кооп режиме роли фиксированы
...
std::string TitanAI::getFireModeBanner() const {
    if (m_IsGunnerMounted) return "CO-OP SEAT SPLIT";
    switch (m_FireMode) {
        case CockpitFireMode::CannonOnly:      return "CANNON ONLY";
        case CockpitFireMode::TeammateMGOnly:  return "TURRET MG ONLY";
        case CockpitFireMode::DuplexTwinSalvo: return "DUPLEX [TWIN SALVO]";
    }
    return "UNKNOWN";
}
...
void TitanAI::triggerReMap(GameState& gs, AncientLoadout newWeapon) {
    if (m_VortexActive || m_IsReMapping) return;
    if (m_ActiveLoadout == newWeapon) return;
