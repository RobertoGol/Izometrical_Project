...
WorkstationUIState WorkstationManager::OpenUI(int objectID)
{
    WorldWorkstation *station = FindByObjectID(objectID);
    return OpenUI(station);
}

WorkstationUIState WorkstationManager::OpenUI(WorldWorkstation *station)
{
    if (!station)
    {
        WorkstationUIState error;
        error.statusMessage = "Station not found.";
        return error;
    }
    return Workstations::OpenWorkstationUI(station);
}
